...
if (!class_exists('Civi_Ajax_Include')) {
	...
	class Civi_Ajax_Include
	{
		...
		function get_login_user()
		{
			...
			if($enable_status){
                ...
                if($user_status === 'approve'){
                    ...
                    $msg = esc_html__('Account is waiting for admin approve', 'civi');
		...
		function get_register_user()
		{
            ...
            //$enable_verify_phone  = Civi_Helper::civi_get_option('enable_verify_phone');
			$redirect_for_admin  = Civi_Helper::civi_get_option('redirect_for_admin');
			$redirect_for_employer  = Civi_Helper::civi_get_option('redirect_for_employer');
			$redirect_for_candidate  = Civi_Helper::civi_get_option('redirect_for_candidate');
...
//                        if($enable_verify_phone){
//                            Civi_Helper::civi_get_verify_phone_number($phone);
//                        }
...
//                    }
		...
		function verify_code()
		{
			...
			if($enable_verify_user){
                if (empty($_COOKIE[trim($user_login)]) || (intval($verify_code) !== intval($_COOKIE[trim($user_login)]))) {
                    $msg = esc_html__('The code gmail is incorrect or has expired.', 'civi');
...
//            if($enable_verify_phone){
//                session_start();
//                if ((empty($verify_code_phone) || empty($_SESSION['verify_phone_code'])) || (intval($verify_code_phone) !==  intval($_SESSION['verify_phone_code']))) {
//                    $msg  = esc_html__('Invalid verification phone number code.', 'civi');
//                    echo json_encode(array('success' => false, 'messages' => $msg, 'verify_code_phone'=> $verify_code_phone,'section'=> $_SESSION['verify_phone_code'],'class' => 'text-error', 'url_redirect' => $url_redirect));
//                    wp_die();
//                }
//            }
