    ...
    public function modify_list_row_actions($actions, $post)
    {
        ...
        if ($post->post_type == 'candidate') {
            if (in_array($post->post_status, array('pending'))) {
                $actions['candidate-approve'] = '<a href="' . wp_nonce_url(add_query_arg('approve_candidate', $post->ID), 'approve_candidate') . '">' . esc_html__('Approve', 'civi-framework') . '</a>';
            }
        }
        return $actions;
    }
...
?>
    ...
    public function add_badge_menu() {
        global $menu;
        $candidate_count = wp_count_posts( 'candidate' )->pending;
