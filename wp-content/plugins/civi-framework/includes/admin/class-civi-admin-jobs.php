...
if (!class_exists('Civi_Admin_Jobs')) {
    ...
    class Civi_Admin_Jobs
    {
        ...
        {
            $jobs_gender_url_slug = civi_get_option('jobs_gender_url_slug');
...
?>
		...
		public function auto_description_generate(){
			...
			$ai_model = civi_get_option('ai_model');
			$ai_temperature = civi_get_option('ai_temperature');
			...
			if ( is_wp_error( $response ) ) {
				...
				if ( isset( $response_body->error->message ) ) {
					if( $response_body->error->code === 'invalid_api_key' ){
						echo json_encode(array('success' => false, 'response' => $response_body, 'message' => esc_html__('Invalid API Key', 'civi-framework')));
					} else {
